...
All input/output happens on google sheets.

Input: urls of a specific startup of interest (databases, linkedin, job-finding websites, etc.), added manually 

Output: standardized data for each column (startup summary, number of employees, funds, job requests, etc.), generated by web crawling + LLM (or agents ?)

updates periodically (using Gihub actions?)

---

multiple sheets. each company has a unique id

1st sheet: hard facts of company
2nd sheet: company -> summary, employee count, funds, last_updated, ...
3rd sheet: company -> employee count


---

```
* * * * * /usr/bin/python3 /Users/easonchiou/STUFF/code/TEST4.py
